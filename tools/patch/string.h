...
struct cl_heap_string : public cl_heap {
 ...
 private:
  ...
  friend cl_heap_string* cl_make_heap_string(const char* ptr,
                                             unsigned long len);
  friend const cl_string operator+(const cl_string& str1,
                                   const cl_string& str2);
...
struct cl_string : public cl_gcpointer {
 public:
  ...
  const char* asciz() const { return &((cl_heap_string*)pointer)->data[0]; }
  // Return the length (number of characters).
  unsigned long size() const { return ((cl_heap_string*)pointer)->length; }
  ...
  char operator[](unsigned int i) const { return operator[]((unsigned long)i); }
  char operator[](long i) const { return operator[]((unsigned long)i); }
  char operator[](int i) const { return operator[]((unsigned long)i); }
  // Constructors.
  cl_string();
...
// Length.
inline unsigned long strlen(const cl_string& str) { return str.size(); }
...
inline bool equal(const cl_string& str1, const cl_string& str2) {
  return str1.size() == str2.size() && !strcmp(str1.asciz(), str2.asciz());
}
...
extern const cl_string cl_fgetline(std::istream& stream, int n,
                                   char delim = '\n');
